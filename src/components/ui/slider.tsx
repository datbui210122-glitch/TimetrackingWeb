...
function Slider({
  ...
  const numberOfThumbs = React.useMemo(() => {
    if (value && Array.isArray(value)) return value.length;
    if (defaultValue && Array.isArray(defaultValue)) return defaultValue.length;
    return 1;
  }, [value, defaultValue]);

  // Conditionally create props for value and defaultValue
  // This ensures they are only passed if they are not undefined
  ...
  return (
    <SliderPrimitive.Root
      data-slot="slider"
      // Apply the conditionally created props here
      {...defaultValueProps}
      {...valueProps}
      min={min}
      max={max}
    ...
    >
      ...
      {Array.from({ length: numberOfThumbs }, (_, index) => (
